...
function SkinCancerPrediction() {
  ...
  const fetchDiseaseFacts = async (prediction) => {
    try {
      const response = await fetch(
        `/api/disease-facts?disease=${encodeURIComponent(prediction)}`
      );
      if (!response.ok) {
  ...
  const generatePDF = async () => {
    ...
    pdf.save("lung-cancer-report.pdf");
    if (downloadButton) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await fetchDiseaseFacts(data.prediction);
      await generatePDF();
    ...
    <div className="min-h-screen py-20 bg-[#F0F4F8] flex flex-col items-center justify-center p-4">
      ...
      {reportGenerated && !loading && (
        ...
        >
          ...
          <div className="mb-4">
            <p className="text-lg font-semibold">
              Diagnosis Result: <span className="font-normal">{result}</span>
            </p>
          </div>
          <div className="mb-4">
